...
const narrativeData = [
  { narrative: "AI Work Companion",      penetration: 38, target: 70 },
  { narrative: "AI Note Taker",          penetration: 62, target: 40 },
  { narrative: "Conv. Intelligence",     penetration: 29, target: 65 },
  { narrative: "AI Productivity Tool",   penetration: 44, target: 50 },
  { narrative: "Capture→Extract→Utilize",penetration: 18, target: 60 },
];
const radarData = [
...
const trendingTopics = [
  { topic: "AI Agents at Work",       relevance: 95, plaudIn: true  },
  { topic: "Ambient AI Hardware",     relevance: 88, plaudIn: true  },
  { topic: "Voice-First Productivity",relevance: 82, plaudIn: false },
  { topic: "Meeting Intelligence",    relevance: 79, plaudIn: true  },
  { topic: "AI Work Companions",      relevance: 91, plaudIn: false },
  { topic: "Human-AI Collaboration",  relevance: 85, plaudIn: false },
];
const journalists = [
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
        <MetricTile label="Weekly Mentions"       value={m ? String(m.totalMentions) : "284"}   trend="up"   sub="vs 201 last week" />
        <MetricTile label="Tier-1 Coverage"       value={m ? String(m.tier1Count)    : "38"}    trend="up"   sub="+41% MoM"         />
        <MetricTile label="Sentiment Score"       value="7.2/10"                                trend="up"   sub="+0.4 pts"         />
        <MetricTile label="Share of Voice"        value={m ? `${m.shareOfVoice}%`    : "12%"}   trend="down" sub="vs 14% last week" />
        <MetricTile label="Exec Quotes Picked Up" value="9"                                     trend="up"   sub="across 6 outlets" />
        <MetricTile label="Narrative Match %"     value={m ? `${m.positiveRatio}%`   : "46%"}   trend="up"   sub="AI Work Companion" />
      </div>
...
function NarrativeModule() {
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <Card>
          <CardTitle sub="Current vs target narrative penetration">Plaud Narrative Ownership</CardTitle>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={narrativeData} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" stroke={BORDER} />
              <XAxis type="number" stroke={MUTED} fontSize={10} domain={[0,100]} unit="%" />
              <YAxis type="category" dataKey="narrative" stroke={MUTED} fontSize={9} width={130} />
              <Tooltip contentStyle={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 8 }} />
              <Bar dataKey="penetration" fill={ACCENT}    name="Current %" radius={[0,4,4,0]} />
              <Bar dataKey="target"      fill="#2a2d3a"   name="Target %"  radius={[0,4,4,0]} />
            </BarChart>
          </ResponsiveContainer>
        </Card>
        <Card>
      ...
      <Card>
        <CardTitle sub="Which AI stories are breaking — and is Plaud in them?">AI Industry Narrative Landscape</CardTitle>
          ...
          {trendingTopics.map(t => (
            <div key={t.topic} style={{ background: "#13151f", border: `1px solid ${t.plaudIn ? ACCENT+"55" : BORDER}`, borderRadius: 10, padding: "12px 14px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                ...
                <span style={{ color: t.plaudIn ? "#10b981" : "#f43f5e", fontSize: 10, fontWeight: 700, whiteSpace: "nowrap", marginLeft: 6 }}>{t.plaudIn ? "IN" : "OUT"}</span>
              </div>
              ...
              {!t.plaudIn && <div style={{ color: "#f59e0b", fontSize: 10, marginTop: 6 }}>Pitch opportunity</div>}
            </div>
          ))}
        </div>
      </Card>
...
function Tier1Module({ coverage }: { coverage: any[] }) {
  ...
  const filtered = filter === "All" ? data : data.filter((d: any) => d.co === filter || d.company === filter);
