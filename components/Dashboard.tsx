...
// ─── 品牌色系 ────────────────────────────────────────────────
const BG     = "#FFFFFF";        // 页面背景
const CARD   = "#F2EFEB";        // 卡片背景（浅暖灰）
const CARD2  = "#FFFFFF";        // 嵌套卡片背景
const BORDER = "#E0DBD5";        // 边框
const TEXT   = "#413D3B";        // 主文字（深暖灰）
const MUTED  = "#8A837E";        // 次要文字
const ACCENT = "#8F53ED";        // Victory Purple（主色）
const GREEN  = "#21EF6A";        // Amplify Green（强调）
const BLUE   = "#00D0FF";        // Clarity Blue（辅助）
const GRAD   = "linear-gradient(135deg, #21EF6A, #00D0FF, #8F53ED)"; // 品牌渐变

const TREND_COLORS = ["#8F53ED","#00D0FF","#21EF6A","#413D3B","#f43f5e","#f59e0b"];

const REGIONS = [
  { key: "us",   label: "US",   flag: "🇺🇸", color: "#8F53ED" },
  { key: "eu",   label: "EU",   flag: "🇪🇺", color: "#00D0FF" },
  { key: "jp",   label: "JP",   flag: "🇯🇵", color: "#21EF6A" },
  { key: "apac", label: "APAC", flag: "🌏",  color: "#413D3B" },
];

const sentColors: Record<string, string> = {
  Positive: "#21EF6A", Neutral: "#8F53ED", Negative: "#f43f5e",
  positive: "#21EF6A", neutral: "#8F53ED", negative: "#f43f5e",
};
const statusColors: Record<string, string> = {
  Engaged: "#21EF6A", Warm: "#f59e0b", Cold: "#f43f5e",
  "In Review": "#8F53ED", Sent: "#00D0FF", Drafted: "#f59e0b",
  Ideation: "#8A837E", High: "#f43f5e", Medium: "#f59e0b",
};
...
const StatusBadge = ({ s }: { s: string }) => (
  <span style={{ background:(statusColors[s]||MUTED)+"18", color:statusColors[s]||MUTED, border:`1px solid ${(statusColors[s]||MUTED)}33`, borderRadius:4, padding:"2px 8px", fontSize:11, fontWeight:600 }}>{s}</span>
);
...
const MetricTile = ({ label, value, trend, sub, highlight, color }: { label:string; value:string; trend?:string; sub?:string; highlight?:boolean; color?:string }) => (
  <div style={{ background:CARD2, border:`1px solid ${highlight?(color||ACCENT)+"55":BORDER}`, borderRadius:12, padding:"16px 18px", boxShadow: highlight?"0 2px 12px "+((color||ACCENT)+"22"):"none" }}>
    <div style={{ color:MUTED, fontSize:11, marginBottom:4, textTransform:"uppercase", letterSpacing:"0.05em" }}>{label}</div>
    <div style={{ color:color||TEXT, fontSize:24, fontWeight:800 }}>{value}</div>
    {trend && <div style={{ color:trend==="up"?"#21EF6A":"#f43f5e", fontSize:11, marginTop:3, fontWeight:600 }}>{trend==="up"?"▲":"▼"} {sub}</div>}
    {!trend && sub && <div style={{ color:MUTED, fontSize:11, marginTop:3 }}>{sub}</div>}
  </div>
);
const GradButton = ({ onClick, disabled, loading, loadingText, children }: { onClick:()=>void; disabled?:boolean; loading?:boolean; loadingText?:string; children:React.ReactNode }) => (
  <button onClick={onClick} disabled={disabled||loading}
    style={{ background:loading?BORDER:GRAD, color:loading?MUTED:"#fff", border:"none", borderRadius:8, padding:"8px 18px", fontSize:12, fontWeight:700, cursor:(disabled||loading)?"not-allowed":"pointer", opacity:(disabled||loading)?0.6:1, whiteSpace:"nowrap" }}>
  ...
  return (
    <Card>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
        ...
        <span style={{ background:ACCENT+"18", color:ACCENT, border:`1px solid ${ACCENT}33`, borderRadius:6, padding:"2px 8px", fontSize:10, fontWeight:700 }}>Q1 2026</span>
      </div>
      <div style={{ color:MUTED, fontSize:11, marginBottom:10 }}>Current vs target narrative penetration</div>
      <div style={{ marginBottom:16, padding:"10px 14px", background:CARD2, border:`1px solid ${BORDER}`, borderRadius:10, fontSize:11, color:MUTED, lineHeight:1.8 }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        {data.map((row,idx)=>(
          <div key={row.narrative}>
              ...
              <div style={{display:"flex",gap:10,alignItems:"center"}}>
                {editing?.idx===idx&&editing?.field==="current"?(
                  <input autoFocus value={editVal} onChange={e=>setEditVal(e.target.value)} onBlur={commitEdit} onKeyDown={e=>e.key==="Enter"&&commitEdit()}
                    style={{width:48,background:CARD2,border:`1px solid ${ACCENT}`,borderRadius:4,color:TEXT,fontSize:12,fontWeight:700,textAlign:"center",padding:"2px 4px",outline:"none"}}/>
                ):(
                ...
                {editing?.idx===idx&&editing?.field==="target"?(
                  <input autoFocus value={editVal} onChange={e=>setEditVal(e.target.value)} onBlur={commitEdit} onKeyDown={e=>e.key==="Enter"&&commitEdit()}
                    style={{width:48,background:CARD2,border:`1px solid ${MUTED}`,borderRadius:4,color:TEXT,fontSize:12,fontWeight:700,textAlign:"center",padding:"2px 4px",outline:"none"}}/>
                ):(
            ...
            <div style={{position:"relative",height:6,background:BORDER,borderRadius:4}}>
              <div style={{position:"absolute",left:0,top:0,width:`${row.target}%`,height:"100%",background:BORDER,borderRadius:4}}/>
              <div style={{position:"absolute",left:0,top:0,width:`${row.current}%`,height:"100%",background:row.current>=row.target?GREEN:ACCENT,borderRadius:4,transition:"width 0.3s"}}/>
            </div>
            <div style={{marginTop:4,fontSize:10,color:row.current>=row.target?GREEN:"#f59e0b"}}>
...
function JournalistTracker({ coverage }: { coverage: any[] }) {
  ...
  const priColors: Record<string,string> = {High:"#f43f5e",Medium:"#f59e0b",Low:GREEN};
  return (
    <div style={{display:"grid",gap:16}}>
      <Card>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          ...
          <span style={{background:GREEN+"22",border:`1px solid ${GREEN}44`,borderRadius:6,padding:"2px 10px",color:"#1a7a3a",fontSize:11,fontWeight:700}}>{autoJournalists.length} found</span>
        </div>
        {autoJournalists.length===0?(
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {autoJournalists.map((j:any)=>(
              <div key={j.name} style={{background:CARD2,border:`1px solid ${ACCENT}33`,borderRadius:10,padding:"10px 14px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                  <div><span style={{color:TEXT,fontSize:12,fontWeight:700}}>{j.name}</span><span style={{color:MUTED,fontSize:11}}> · {j.outlet}</span></div>
                  <span style={{background:GREEN+"22",color:"#1a7a3a",borderRadius:4,padding:"2px 8px",fontSize:10,fontWeight:600}}>Has covered Plaud</span>
                </div>
      ...
      <Card>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          ...
          <button onClick={()=>setShowAdd(!showAdd)} style={{background:ACCENT,color:"#fff",border:"none",borderRadius:8,padding:"6px 14px",fontSize:11,fontWeight:700,cursor:"pointer"}}>+ Add</button>
        </div>
        {showAdd&&(
          <div style={{background:CARD2,border:`1px solid ${ACCENT}44`,borderRadius:10,padding:14,marginBottom:12,display:"grid",gridTemplateColumns:"1fr 1fr 1fr 60px",gap:8,alignItems:"end"}}>
            {[{label:"Name",key:"name",placeholder:"e.g. Lauren Goode"},{label:"Outlet",key:"outlet",placeholder:"e.g. Wired"},{label:"Beat",key:"beat",placeholder:"e.g. AI Hardware"}].map(f=>(
              ...
              <input value={(newJ as any)[f.key]} onChange={e=>setNewJ(p=>({...p,[f.key]:e.target.value}))} placeholder={f.placeholder}
                style={{width:"100%",background:CARD,border:`1px solid ${BORDER}`,borderRadius:6,padding:"6px 8px",color:TEXT,fontSize:11,outline:"none"}}/></div>
            ))}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {manualList.map((j,i)=>(
            <div key={i} style={{background:CARD2,border:`1px solid ${BORDER}`,borderRadius:10,padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      ...
      <Card>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          ...
          <GradButton onClick={generateRecs} loading={loading} loadingText="Generating...">⚡ Generate Pitch Recs</GradButton>
        </div>
        ...
        {aiRecs.length>0&&(
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {aiRecs.map((rec:any,i:number)=>(
              <div key={i} style={{background:CARD2,border:`1px solid ${(priColors[rec.priority]||BORDER)}33`,borderRadius:10,padding:"12px 14px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                  ...
                  <span style={{background:(priColors[rec.priority]||MUTED)+"18",color:priColors[rec.priority]||MUTED,border:`1px solid ${(priColors[rec.priority]||MUTED)}33`,borderRadius:4,padding:"2px 8px",fontSize:10,fontWeight:700}}>{rec.priority} Priority</span>
                </div>
                <div style={{color:BLUE,fontSize:11,fontWeight:600,marginBottom:4}}>Pitch angle: {rec.angle}</div>
...
function PulseModule({
  ...
  return (
    <div style={{display:"grid",gap:16}}>
      {!hasAnyData && (
        <div style={{padding:"10px 16px",background:"#fff8e6",border:"1px solid #f59e0b44",borderRadius:10,fontSize:11,color:"#92620a"}}>
          ...
          {REGIONS.map(r => {
            ...
            return (
              <MetricTile key={r.key}
                ...
                highlight={hasData} color={r.color}
              />
            );
          })}
        </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {REGIONS.map(r => {
              ...
              const score = stats && stats.total > 0
                ...
                : null;
              return (
                <div key={r.key} style={{background:CARD2,borderRadius:10,padding:"12px 14px",border:`1px solid ${r.color}33`}}>
                  <div style={{color:r.color,fontSize:10,fontWeight:700,textTransform:"uppercase"}}>{r.flag} {r.label}</div>
                  <div style={{color:TEXT,fontSize:22,fontWeight:800,marginTop:2}}>{score ? `${score}/10` : "—"}</div>
          ...
          {REGIONS.map(r => {
            ...
            return (
              <div key={r.key}
                ...
                style={{border:`2px dashed ${dragging===r.key?r.color:hasData?r.color+"66":BORDER}`,borderRadius:12,padding:"18px 12px",textAlign:"center",background:dragging===r.key?r.color+"11":hasData?r.color+"08":CARD2,transition:"all 0.15s",cursor:"pointer"}}
              >
                <div style={{fontSize:24,marginBottom:6}}>{r.flag}</div>
    ...
    <div style={{display:"grid",gap:16}}>
        ...
        {!industryLoading&&industryData&&(
          <div style={{display:"grid",gap:12}}>
            <div style={{background:GRAD,borderRadius:12,padding:"14px 18px"}}>
              ...
              <div style={{color:"#fff",fontSize:13,fontWeight:700,lineHeight:1.5}}>{industryData.headline}</div>
              ...
              {(industryData.reports||[]).map((r:any,i:number)=>(
                <div key={i} style={{background:CARD2,border:`1px solid ${(categoryColors[r.category]||BORDER)}33`,borderRadius:10,padding:"12px 14px"}}>
                  ...
                  <div style={{color:MUTED,fontSize:10,marginBottom:6}}>
                    {r.url?<a href={r.url} target="_blank" rel="noopener noreferrer" style={{color:BLUE,textDecoration:"none"}}>{r.source}</a>:r.source} · {r.date}
                  </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {(thoughtData.leaders||[]).map((l:any,i:number)=>(
                <div key={i} style={{background:CARD2,border:`1px solid ${(sentimentColors[l.sentiment]||BORDER)}33`,borderRadius:10,padding:"12px 14px"}}>
                  ...
                  <div style={{color:ACCENT,fontSize:11,fontStyle:"italic",marginBottom:6,lineHeight:1.5}}>"{l.quote}"</div>
                  <div style={{color:MUTED,fontSize:10,marginBottom:6}}>
                    {l.url?<a href={l.url} target="_blank" rel="noopener noreferrer" style={{color:BLUE,textDecoration:"none"}}>{l.source}</a>:l.source} · {l.date}
                  </div>
      ...
      <Card>
        <div style={{marginBottom:16}}>
          ...
          <div style={{marginTop:8,display:"inline-flex",alignItems:"center",gap:6,background:CARD2,border:`1px solid ${BORDER}`,borderRadius:8,padding:"6px 12px"}}>
            <span style={{color:GREEN,fontSize:11,fontWeight:700}}>IN</span><span style={{color:MUTED,fontSize:11}}>=</span>
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <RadarChart data={radarData}>
              ...
              <Radar name="Plaud"    dataKey="plaud" stroke={ACCENT} fill={ACCENT} fillOpacity={0.2}/>
              <Radar name="Otter.ai" dataKey="otter" stroke={BLUE}   fill={BLUE}   fillOpacity={0.1}/>
        ...
        <Card>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {[
              ...
              {co:"reMarkable",signal:"Product review cycle in The Verge — physical device framing",  impact:"Low",        color:GREEN},
              {co:"Humane AI", signal:"Negative press cycle — AI wearables overpromise",              impact:"Opportunity",color:"#f59e0b"},
            ].map(c=>(
              <div key={c.co} style={{background:CARD2,border:`1px solid ${BORDER}`,borderRadius:10,padding:"10px 12px"}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                  <span style={{color:BLUE,fontSize:12,fontWeight:700}}>{c.co}</span>
    ...
    <div style={{display:"grid",gap:16}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <Card>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {pitches.map(p=>(
              <div key={p.angle} style={{background:CARD2,border:`1px solid ${BORDER}`,borderRadius:10,padding:"10px 12px"}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                  <span style={{color:BLUE,fontSize:11,fontWeight:600}}>{p.outlet}</span>
        ...
        <Card>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {upcomingEvents.map(e=>(
              <div key={e.event} style={{background:CARD2,border:`1px solid ${BORDER}`,borderRadius:10,padding:"10px 12px",display:"flex",gap:12,alignItems:"center"}}>
                <div style={{background:ACCENT+"18",border:`1px solid ${ACCENT}33`,borderRadius:8,padding:"6px 10px",textAlign:"center",minWidth:50}}>
                  <div style={{color:ACCENT,fontSize:11,fontWeight:700}}>{e.date}</div>
                </div>
                <div>
                  <div style={{color:TEXT,fontSize:12,fontWeight:600}}>{e.event}</div>
                  <div style={{color:GREEN,fontSize:11,fontWeight:500}}>→ {e.opportunity}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <Card>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {[
            ...
            <div key={q.quote} style={{background:CARD2,border:`1px solid ${BORDER}`,borderRadius:10,padding:"12px 14px"}}>
      ...
      <Card>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          ...
          <div style={{display:"flex",gap:8}}>
            <GradButton onClick={generate} loading={loading} loadingText="Analyzing...">Generate Brief</GradButton>
            <button onClick={sendReport} disabled={sending}
              style={{background:sending?BORDER:GREEN,color:sending?MUTED:TEXT,border:"none",borderRadius:8,padding:"8px 18px",fontSize:12,fontWeight:700,cursor:sending?"not-allowed":"pointer",opacity:sending?0.6:1}}>
        ...
        {insights&&!loading&&(
          <div style={{display:"grid",gap:14}}>
            <div style={{background:GRAD,borderRadius:12,padding:16}}>
              <div style={{color:"rgba(255,255,255,0.7)",fontSize:11,fontWeight:700,textTransform:"uppercase",marginBottom:6}}>Executive Summary</div>
              <div style={{color:"#fff",fontSize:13,lineHeight:1.6}}>{insights.summary}</div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <div style={{background:"#fff0f0",border:"1px solid #f43f5e33",borderRadius:10,padding:"12px 14px"}}>
              ...
              <div style={{background:GREEN+"12",border:`1px solid ${GREEN}33`,borderRadius:10,padding:"12px 14px"}}>
                <div style={{color:"#1a7a3a",fontSize:10,fontWeight:700,marginBottom:4}}>TOP OPPORTUNITY</div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {(insights.actions||[]).map((a:string,i:number)=>(
                  <div key={i} style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                    <div style={{width:18,height:18,border:`2px solid ${ACCENT}`,borderRadius:4,flexShrink:0,marginTop:1}}/>
        ...
        {sendMsg && (
          <div style={{marginTop:12,padding:"10px 14px",background:sendMsg.startsWith("✓")?GREEN+"15":"#fff0f0",border:`1px solid ${sendMsg.startsWith("✓")?GREEN+"44":"#f43f5e33"}`,borderRadius:10,fontSize:11,color:sendMsg.startsWith("✓")?"#1a7a3a":"#f43f5e"}}>
            {sendMsg}
          </div>
        )}
      </Card>
    ...
    <div style={{display:"grid",gap:16}}>
      {tier1Rows.length===0&&(
        <Card><div style={{textAlign:"center",padding:"32px 0",color:MUTED}}>
          ...
          <div style={{fontSize:12}}>Upload a Meltwater CSV below to see articles with Reach &gt;1M</div>
        </div></Card>
      )}
      {tier1Rows.length>0&&(
        <>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
            ...
            <MetricTile label="Positive"        value={String(pos)} trend="up"   sub={`${Math.round((pos/tier1Rows.length)*100)}% of total`} color={GREEN}/>
            <MetricTile label="Negative"        value={String(neg)} trend="down" sub={`${Math.round((neg/tier1Rows.length)*100)}% of total`} color="#f43f5e"/>
              ...
              <div style={{display:"flex",gap:6}}>
                {sentFilters.map(f=>(
                  <button key={f} onClick={()=>setFilter(f)}
                    style={{background:filter===f?(f==="Positive"?GREEN:f==="Negative"?"#f43f5e":f==="Neutral"?ACCENT:ACCENT):CARD2,color:filter===f?(f==="Positive"?TEXT:"#fff"):MUTED,border:`1px solid ${filter===f?BORDER:BORDER}`,borderRadius:6,padding:"4px 12px",fontSize:11,fontWeight:600,cursor:"pointer"}}>{f}</button>
                ))}
              </div>
            </div>
              ...
              {filtered.map((item,i)=>(
                <div key={i} style={{background:CARD2,border:`1px solid ${BORDER}`,borderRadius:10,padding:"12px 14px"}}>
                  ...
                  <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap" as const}}>
                    {item.outlet&&<span style={{color:BLUE,fontSize:11,fontWeight:600}}>{item.outlet}</span>}
                    {item.date&&<span style={{color:MUTED,fontSize:10}}>{item.date}</span>}
                    {item.reach&&<span style={{background:ACCENT+"15",color:ACCENT,borderRadius:4,padding:"1px 6px",fontSize:10,fontWeight:600}}>Reach: {Number(item.reach).toLocaleString()}</span>}
      ...
      <Card>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          ...
          {tier1Rows.length>0&&(
            <div style={{display:"flex",gap:8}}>
              <div style={{background:GREEN+"18",border:`1px solid ${GREEN}44`,borderRadius:8,padding:"4px 12px",color:"#1a7a3a",fontSize:11,fontWeight:700}}>{pos} Positive</div>
              <div style={{background:"#fff0f0",border:"1px solid #f43f5e44",borderRadius:8,padding:"4px 12px",color:"#f43f5e",fontSize:11,fontWeight:700}}>{neg} Negative</div>
              <div style={{background:CARD2,border:`1px solid ${BORDER}`,borderRadius:8,padding:"4px 12px",color:MUTED,fontSize:11,fontWeight:700}}>{neu} Neutral</div>
            </div>
          )}
        </div>
        <div style={{marginBottom:12,padding:"10px 14px",background:CARD2,border:`1px solid ${BORDER}`,borderRadius:10,fontSize:11,color:MUTED,lineHeight:1.8}}>
          <div style={{color:TEXT,fontWeight:600,marginBottom:4}}>How to export from Meltwater:</div>
          <div>1. Search <strong style={{color:ACCENT}}>"Plaud"</strong> → Set date range → Click <strong style={{color:BLUE}}>Export → CSV</strong></div>
          <div>2. Upload below — articles with <strong style={{color:ACCENT}}>Reach &gt; 1,000,000</strong> will be shown as Tier 1</div>
          <div>3. This data also updates the <strong style={{color:BLUE}}>Pulse</strong> tab metrics automatically</div>
        </div>
        <div onDragOver={e=>{e.preventDefault();setDragging(true);}} onDragLeave={()=>setDragging(false)}
          ...
          style={{border:`2px dashed ${dragging?ACCENT:BORDER}`,borderRadius:12,padding:"24px 20px",textAlign:"center",background:dragging?ACCENT+"08":CARD2,transition:"all 0.15s",cursor:"pointer"}}>
          <div style={{fontSize:28,marginBottom:8}}>📊</div>
          <div style={{color:TEXT,fontSize:13,fontWeight:600,marginBottom:4}}>Drop Meltwater CSV here or click to upload</div>
...
function BrandAwarenessModule({ keywordData,pageData,uploadedAt,onLoad,trendsData,trendsKeywords,trendsUploadedAt,onTrendsLoad }: {
  ...
  return (
    <div style={{display:"grid",gap:16}}>
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={trendsData}>
              ...
              <Tooltip contentStyle={{background:CARD2,border:`1px solid ${BORDER}`,borderRadius:8,fontSize:11}} formatter={(val:any,name:any)=>[`${val}`,name]}/>
          ...
          <div style={{display:"grid",gridTemplateColumns:`repeat(${Math.min(trendsKeywords.length,3)},1fr)`,gap:10,marginTop:14}}>
            {trendsKeywords.map((kw,i)=>{
              ...
              return (
                <div key={kw} style={{background:CARD2,border:`1px solid ${TREND_COLORS[i%TREND_COLORS.length]}33`,borderRadius:10,padding:"12px 14px"}}>
                  <div style={{color:TREND_COLORS[i%TREND_COLORS.length],fontSize:11,fontWeight:700,marginBottom:6}}>{kw}</div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:4}}>
                    <div><div style={{color:MUTED,fontSize:9,textTransform:"uppercase"}}>Peak</div><div style={{color:TEXT,fontSize:16,fontWeight:800}}>{peak}</div></div>
                    <div><div style={{color:MUTED,fontSize:9,textTransform:"uppercase"}}>Avg</div><div style={{color:TEXT,fontSize:16,fontWeight:800}}>{avg}</div></div>
                    <div><div style={{color:MUTED,fontSize:9,textTransform:"uppercase"}}>Latest</div><div style={{color:TEXT,fontSize:16,fontWeight:800}}>{latest}</div></div>
                  </div>
                </div>
              );
            })}
      ...
      {!hasData&&<Card><div style={{textAlign:"center",padding:"32px 0",color:MUTED}}><div style={{fontSize:36,marginBottom:12}}>📊</div><div style={{fontSize:13,fontWeight:600,marginBottom:6,color:TEXT}}>No GA4 data uploaded yet</div><div style={{fontSize:12}}>Follow the export guide below.</div></div></Card>}
      {hasData&&(
        <>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
            <MetricTile label="Brand Queries"     value={String(brandKws.length)}                sub="plaud / notepin terms"/>
            <MetricTile label="Brand Clicks"      value={totalBrandClicks.toLocaleString()}      trend="up" sub="last 28 days" color={GREEN}/>
            <MetricTile label="Brand Impressions" value={totalBrandImpressions.toLocaleString()} trend="up" sub="last 28 days" color={ACCENT}/>
                ...
                <ResponsiveContainer width="100%" height={240}>
                  <BarChart data={keywordChart}>
                    ...
                    <Bar dataKey="clicks" name="Clicks" radius={[4,4,0,0]}>
                      {keywordChart.map((entry,i)=><Cell key={i} fill={entry.isBrand?ACCENT:BORDER}/>)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </Card>
              ...
              <Card>
                ...
                <ResponsiveContainer width="100%" height={240}>
                  <BarChart data={pageChart} layout="vertical">
                    ...
                    <Tooltip contentStyle={{background:CARD2,border:`1px solid ${BORDER}`,borderRadius:8}}/>
                    <Legend wrapperStyle={{fontSize:11}}/>
                    <Bar dataKey="views" fill={BLUE}   name="Views" radius={[0,4,4,0]}/>
                ...
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                  <thead><tr style={{borderBottom:`2px solid ${BORDER}`}}>
                    {["Query","Clicks","Impressions","CTR","Avg. Position","Type"].map(h=>(
                      <th key={h} style={{color:MUTED,padding:"8px 10px",textAlign:h==="Query"?"left":"center",fontWeight:600}}>{h}</th>
                    ))}
                  </tr></thead>
                  <tbody>
                    {keywordData.slice(0,15).map((r,i)=>{
                      ...
                      return (
                        <tr key={i} style={{borderBottom:`1px solid ${BORDER}`,background:isBrand?ACCENT+"08":"transparent"}}>
                          ...
                          <td style={{color:BLUE,padding:"8px 10px",textAlign:"center"}}>{r.CTR}</td>
                          <td style={{color:TEXT,padding:"8px 10px",textAlign:"center"}}>{r.Position}</td>
                          <td style={{padding:"8px 10px",textAlign:"center"}}>
                            <span style={{background:isBrand?ACCENT+"18":"#f2efeb",color:isBrand?ACCENT:MUTED,borderRadius:4,padding:"2px 8px",fontSize:10,fontWeight:600}}>{isBrand?"Brand":"Non-Brand"}</span>
                          </td>
                        </tr>
                      );
                    })}
      ...
      <Card>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          ...
          {hasData&&(
            <div style={{display:"flex",gap:8}}>
              <div style={{background:GREEN+"18",border:`1px solid ${GREEN}44`,borderRadius:8,padding:"6px 14px",color:"#1a7a3a",fontSize:11,fontWeight:700}}>Brand Clicks: {totalBrandClicks.toLocaleString()}</div>
              <div style={{background:ACCENT+"18",border:`1px solid ${ACCENT}44`,borderRadius:8,padding:"6px 14px",color:ACCENT,fontSize:11,fontWeight:700}}>Brand Impressions: {totalBrandImpressions.toLocaleString()}</div>
            </div>
          )}
        </div>
        <div style={{marginBottom:14,padding:"10px 14px",background:CARD2,border:`1px solid ${BORDER}`,borderRadius:10,fontSize:11,color:MUTED,lineHeight:1.8}}>
          <div style={{color:TEXT,fontWeight:600,marginBottom:6}}>How to export from GA4:</div>
          <div>1. <span style={{color:BLUE}}>Keywords CSV:</span> GA4 → Reports → Acquisition → Search Console → Queries → Last 28 days → Download CSV</div>
        ...
        <div onDragOver={e=>{e.preventDefault();setDragging(true);}} onDragLeave={()=>setDragging(false)}
          ...
          <div style={{color:MUTED,fontSize:11}}>Supports <strong style={{color:BLUE}}>ga4-keywords.csv</strong> and <strong style={{color:ACCENT}}>ga4-pages.csv</strong></div>
        ...
        <div style={{display:"flex",gap:10,marginTop:10}}>
          <div style={{flex:1,background:CARD,border:`1px solid ${keywordData.length?BLUE+"66":BORDER}`,borderRadius:8,padding:"8px 12px",fontSize:11}}>
            <span style={{color:keywordData.length?GREEN:MUTED}}>{keywordData.length?"✓":"○"}</span>
            <span style={{color:MUTED,marginLeft:6}}>Keywords CSV</span>
            {keywordData.length>0&&<span style={{color:BLUE,marginLeft:6}}>{keywordData.length} queries loaded</span>}
          </div>
          <div style={{flex:1,background:CARD,border:`1px solid ${pageData.length?ACCENT+"66":BORDER}`,borderRadius:8,padding:"8px 12px",fontSize:11}}>
            <span style={{color:pageData.length?GREEN:MUTED}}>{pageData.length?"✓":"○"}</span>
      ...
      <Card>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          ...
          {hasTrends&&(
            <div style={{display:"flex",gap:6,flexWrap:"wrap" as const}}>
              {trendsKeywords.map((kw,i)=>(
                <span key={kw} style={{background:TREND_COLORS[i%TREND_COLORS.length]+"18",border:`1px solid ${TREND_COLORS[i%TREND_COLORS.length]}44`,borderRadius:6,padding:"3px 10px",color:TREND_COLORS[i%TREND_COLORS.length],fontSize:11,fontWeight:700}}>{kw}</span>
              ))}
            </div>
          )}
        </div>
        <div style={{marginBottom:14,padding:"10px 14px",background:CARD2,border:`1px solid ${BORDER}`,borderRadius:10,fontSize:11,color:MUTED,lineHeight:1.8}}>
          ...
          <div>1. Go to <span style={{color:BLUE}}>trends.google.com</span> → Search "Plaud" + add competitors</div>
          <div>2. Set date range → Click <span style={{color:ACCENT}}>↓ download</span> button</div>
          <div>3. Upload the downloaded <strong style={{color:BLUE}}>multiTimeline.csv</strong> below</div>
        </div>
        <div onDragOver={e=>{e.preventDefault();setDraggingTrends(true);}} onDragLeave={()=>setDraggingTrends(false)}
          ...
          style={{border:`2px dashed ${draggingTrends?GREEN:BORDER}`,borderRadius:12,padding:"24px 20px",textAlign:"center",background:draggingTrends?GREEN+"08":CARD2,transition:"all 0.15s",cursor:"pointer"}}>
          <div style={{fontSize:28,marginBottom:8}}>📈</div>
          <div style={{color:TEXT,fontSize:13,fontWeight:600,marginBottom:4}}>Drop Google Trends CSV here or click to upload</div>
          <div style={{color:MUTED,fontSize:11}}>Supports <strong style={{color:BLUE}}>multiTimeline.csv</strong></div>
...
export default function Dashboard({ initialInsights,initialCoverage,initialMetrics,generatedAt,weekNumber }: DashboardProps) {
  ...
  useState(()=>{
    const load = async () => {
      try {
        ...
        const newPrev: Record<string,{rows:any[];date:string|null}> = {us:{rows:[],date:null},eu:{rows:[],date:null},jp:{rows:[],date:null},apac:{rows:[],date:null}};
        if (usPrev.data)   newPrev.us   = {rows:usPrev.data.rows,   date:usPrev.data.date};
        if (euPrev.data)   newPrev.eu   = {rows:euPrev.data.rows,   date:euPrev.data.date};
        if (jpPrev.data)   newPrev.jp   = {rows:jpPrev.data.rows,   date:jpPrev.data.date};
        if (apacPrev.data) newPrev.apac = {rows:apacPrev.data.rows, date:apacPrev.data.date};
        setRegionPrevData(newPrev);
      } catch {}
      setLoaded(true);
    };
    load();
  ...
  return (
    <div style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',-apple-system,sans-serif",color:TEXT}}>
      ...
      {/* Header */}
      <div style={{background:CARD,borderBottom:`1px solid ${BORDER}`,padding:"14px 28px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <div style={{background:GRAD,borderRadius:10,width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:800,fontSize:18}}>P</div>
          <div>
        ...
        <div style={{background:GREEN+"18",border:`1px solid ${GREEN}44`,borderRadius:20,padding:"5px 14px",color:"#1a7a3a",fontSize:11,fontWeight:700}}>● Live</div>
      </div>

      {/* Tabs */}
      <div style={{background:CARD,padding:"0 28px",borderBottom:`1px solid ${BORDER}`,display:"flex",gap:0,overflowX:"auto"}}>
        {tabs.map((t,i)=>(
          <button key={i} onClick={()=>setTab(i)} style={{background:"none",border:"none",borderBottom:tab===i?`2px solid ${ACCENT}`:"2px solid transparent",color:tab===i?ACCENT:MUTED,padding:"13px 18px",fontSize:12,fontWeight:tab===i?700:500,cursor:"pointer",whiteSpace:"nowrap",transition:"color 0.15s"}}>{t}</button>
        ))}
      </div>
