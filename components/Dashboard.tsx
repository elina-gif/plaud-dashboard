  ...
  const generateRecs = async () => {
    ...
    try {
      ...
      const res  = await fetch("/api/insights", {
        ...
        body: JSON.stringify({ journalistMode: true, journalists: allJ }),
      });
    ...
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <Card>
        ...
        {autoJournalists.length === 0 ? (
          <div style={{ color: MUTED, fontSize: 12, padding: "16px 0", textAlign: "center" }}>
            No journalist data yet — Tier 1 Coverage will auto-populate this section
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {autoJournalists.map((j: any) => (
              <div key={j.name} style={{ background: "#13151f", border: `1px solid ${ACCENT}33`, borderRadius: 8, padding: "10px 14px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                  <div>
                    <span style={{ color: TEXT, fontSize: 12, fontWeight: 700 }}>{j.name}</span>
                    <span style={{ color: MUTED, fontSize: 11 }}> · {j.outlet}</span>
                    <span style={{ color: MUTED, fontSize: 10 }}> · {j.traffic}</span>
                  </div>
      ...
      <Card>
        ...
        {showAdd && (
          <div style={{ background: "#13151f", border: `1px solid ${ACCENT}44`, borderRadius: 8, padding: 14, marginBottom: 12, display: "grid", gridTemplateColumns: "1fr 1fr 1fr 60px", gap: 8, alignItems: "end" }}>
            {[
              { label: "Name", key: "name", placeholder: "e.g. Lauren Goode" },
              { label: "Outlet", key: "outlet", placeholder: "e.g. Wired" },
              { label: "Beat", key: "beat", placeholder: "e.g. AI Hardware" },
            ].map(f => (
              <div key={f.key}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {manualList.map((j, i) => (
            <div key={i} style={{ background: "#13151f", border: `1px solid ${BORDER}`, borderRadius: 8, padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button onClick={() => setManualList(prev => prev.filter((_, idx) => idx !== i))}
                style={{ background: "none", border: "none", color: "#f43f5e", cursor: "pointer", fontSize: 18, padding: "0 4px" }}>×</button>
            </div>
          ))}
        </div>
      </Card>
      ...
      <Card>
        ...
        {aiRecs.length === 0 && !loading && (
          <div style={{ textAlign: "center", padding: "24px 0", color: MUTED }}>
            ...
            <div style={{ fontSize: 12 }}>Click "Generate Pitch Recs" to get AI-powered journalist recommendations based on your library and Tier 1 coverage data.</div>
          </div>
        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {aiRecs.map((rec: any, i: number) => (
              <div key={i} style={{ background: "#13151f", border: `1px solid ${(priorityColors[rec.priority]||BORDER)}33`, borderRadius: 8, padding: "12px 14px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                  <div>
                    <span style={{ color: TEXT, fontSize: 13, fontWeight: 700 }}>{rec.name}</span>
                    <span style={{ color: MUTED, fontSize: 11 }}> · {rec.outlet}</span>
                  </div>
                  <span style={{ background: (priorityColors[rec.priority]||MUTED)+"22", color: priorityColors[rec.priority]||MUTED, border: `1px solid ${(priorityColors[rec.priority]||MUTED)}44`, borderRadius: 4, padding: "2px 8px", fontSize: 10, fontWeight: 700 }}>
...
function BrandAwarenessModule() {
  ...
  const keywordChart = keywordData.slice(0,10).map(r => ({
    name: (r.Query||"").length>22?(r.Query||"").slice(0,20)+"…":(r.Query||""),
    clicks: parseInt(r.Clicks||"0")||0,
    isBrand: (r.Query||"").toLowerCase().includes("plaud")||(r.Query||"").toLowerCase().includes("notepin"),
  }));
  const pageChart = pageData.slice(0,8).map(r => ({
    name: ((r["Page path and screen class"]||r.Page||r.page||"/").replace("https://www.plaud.ai","").slice(0,25))||"/",
    views: parseInt(r.Views||r.views||"0")||0,
      ...
      <Card>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <div>
            ...
            <div style={{ color: MUTED, fontSize: 11, marginTop: 2 }}>{uploadedAt ? `Last uploaded: ${uploadedAt} · Data period: last 28 days` : "Upload your GA4 CSV exports — use Last 28 Days date range"}</div>
          </div>
          {hasData && (
            <div style={{ display:"flex", gap:8 }}>
        ...
        <div onDragOver={e=>{e.preventDefault();setDragging(true);}} onDragLeave={()=>setDragging(false)} onDrop={onDrop}
          ...
          <input id="csv-input" type="file" accept=".csv" multiple style={{ display:"none" }} onChange={e=>Array.from(e.target.files||[]).forEach(handleFile)} />
        </div>
                    ...
                    {pageData.slice(0,10).map((r,i)=>(
                      <tr key={i} style={{ borderBottom:`1px solid ${BORDER}22` }}>
                        <td style={{ color:CYAN, padding:"8px 10px", fontWeight:600, fontSize:11 }}>
                          {(r["Page path and screen class"]||r.Page||r.page||"/").replace("https://www.plaud.ai","")}
                        </td>
                        ...
                        <td style={{ color:MUTED,  padding:"8px 10px", textAlign:"center" }}>{r["Avg. Engagement Time"]||"—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
  ...
  return (
    <div style={{ background: DARK, minHeight: "100vh", fontFamily: "'Inter',-apple-system,sans-serif", color: TEXT }}>
      ...
      <div style={{ padding: 24, maxWidth: 1200, margin: "0 auto" }}>
        ...
        {tab===3 && <ActionModule coverage={initialCoverage||defaultCoverage} />}
